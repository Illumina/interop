...
#define INTEROP_IMAGING_COLUMN_TYPES \
        ...
        INTEROP_TUPLE7(Time,                  metrics::extraction_metric,         date_time,                  Void, Float, ValueType, 0)\
...
#       undef INTEROP_TUPLE7 // Reuse this for another conversion
    private:
        inline static void round(float& val, const size_t n)
        {
            const float mult = powf(10.0f, static_cast<float>(n));
            val = roundf(val*mult)/mult;
        }
        inline static void round(FloatChannelArray& vals, const size_t n)
        {
            for(FloatChannelArray::iterator b = vals.begin(), e = vals.end();b != e;++b) round(*b, n);
        }
        template<typename T>
        inline static void round(T&, const size_t) { }
