                           ...
                           model::summary::run_summary &run) throw( model::index_out_of_bounds_exception )
{
    ...
    for(size_t read=0;read<tmp.size();++read)
    {
        ...
        for(typename cycle_range_tile_t::const_iterator it = tmp[read].begin(), end = tmp[read].end();it != end;++it)
        {
    ...
    for(size_t read=0;read<run.size();++read)
    {
        for (size_t lane = 0; lane < run[read].size(); ++lane)
        {
            ...
            const cycle_range cycle_range_within_read = summary_by_lane_read[read][lane] - first_cycle_index_of_read;
            (run[read][lane].cycle_state().*set_cycle_state_fun)(cycle_range_within_read);
        }
    }
    for(const_max_tile_iterator range_it = tmp_by_tile.begin(), range_end = tmp_by_tile.end();range_it != range_end;
        ++range_it)
